...
class OneDriveShareTransferManager:
    ...
    async def get_session(self):
        async with httpx.AsyncClient(**self.http) as client:
            return client

    async def init_set(self):
        self.ref_path = (
            await self.s.get(self.path)
            .history[0]
            .headers["Location"]
            .split("/")[7]
            .split("&")[0]
    ...
    async def iter_tasks(self):
        self.s = await self.get_session()
        await self.init_set()
        async for url, name, size in self._iter_items(self.ref_path):
